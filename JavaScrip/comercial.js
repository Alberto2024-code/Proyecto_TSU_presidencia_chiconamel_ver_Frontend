...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (inputBuscador) {
        inputBuscador.addEventListener("input", (e) => {
            ...
            // Pintar los resultados filtrados en la tabla
            renderizarTabla(filtrados);
        });
    }
});

async function cargarServiciosComerciales() {
    let respuesta;
    const urlEndpoint = '/api/civiles/comerciales';
    ...
    try {
        ...
        alert("Hubo un problema al cargar el listado de los servicios.");
    }
}
...
function renderizarTabla(lista) {
    ...
    lista.forEach(c => {
        ...
        // Etiqueta dinámica según el tipo de servicio
        const tipoServicioTexto = c.tipo_servicio === 2 || String(c.tipo_servicio).toLowerCase().includes('comercial') 
            ? 'COMERCIAL' 
            : (c.tipo_servicio === 1 ? 'DOMÉSTICO' : (c.tipo_servicio || 'COMERCIAL'));

        fila.innerHTML = `
            <td>${c.id || c.id_ciudadano || '--'}</td>
            <td>${c.nombre || ''}</td>
            <td>${c.apellido_paterno || ''}</td>
            <td>${c.apellido_materno || ''}</td>
            <td>${c.cuenta_no || 'N/A'}</td>
            <td>${c.comunidad || c.nombre_comunidad || ''}</td>
            <td>${c.domicilio || ''}</td>
            <td><span class="badge-domestico">${tipoServicioTexto}</span></td>
